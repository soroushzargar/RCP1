...
def run(
    ...
    r_range = [0, 0.005, 0.01, 0.015, 0.02, 0.03, 0.05, 0.06]
    ...
    smooth_prediction.y_true = smooth_prediction.y_true.to(device)
    ...
    vanilla_results.to_csv(f"{result_folder}/low-r-vanilla_results-{dataset_name}-smooth{smoothing_sigma}-model{model_sigma}-{score_method}-nsamples{n_trial_samples}.csv", index=False)
    ...
    cas_results.to_csv(f"{result_folder}/low-r-cas_results-{dataset_name}-smooth{smoothing_sigma}-model{model_sigma}-{score_method}-nsamples{n_trial_samples}-conf{confidence}.csv", index=False)
    bin_results.to_csv(f"{result_folder}/low-r-bin_results-{dataset_name}-smooth{smoothing_sigma}-model{model_sigma}-{score_method}-nsamples{n_trial_samples}-conf{confidence}.csv", index=False)
