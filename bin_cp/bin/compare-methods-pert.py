...
def run(
    ...
    for coverage_guarantee in coverage_range:
        ...
        for iter_i in range(n_iterations):
            ...
            pred_set = vanilla_cp.predict_from_scores(smooth_scores_pert[eval_mask])
    ...
    vanilla_results.to_csv(f"{result_folder}/vanilla_results-{dataset_name}-smooth{smoothing_sigma}-model{model_sigma}-{score_method}-nsamples{n_trial_samples}-r_{r}-adversarial.csv", index=False)
    ...
    for r in r_range:
        ...
        for coverage_guarantee in coverage_range:
            ...
            for iter_i in tqdm(range(n_iterations)):
                ...
                pred_set_cas = cas_cp.predict_from_scores(smooth_scores_pert[eval_mask])
    ...
    cas_results.to_csv(f"{result_folder}/cas_results-{dataset_name}-smooth{smoothing_sigma}-model{model_sigma}-{score_method}-nsamples{n_trial_samples}-conf{confidence}-r_{r}-adversarial.csv", index=False)
    bin_results.to_csv(f"{result_folder}/bin_results-{dataset_name}-smooth{smoothing_sigma}-model{model_sigma}-{score_method}-nsamples{n_trial_samples}-conf{confidence}-r_{r}-adversarial.csv", index=False)
