...
def run(
    ...
    vanilla_results.to_csv(f"{result_folder}/vanilla_results-time-tr_samples{trial_samples}-NOERROR.csv", index=False)
    vanilla_results[vanilla_results["coverage_guarantee"] == 0.9].mean()

    vanilla_bin_cp = BinCP(nominal_coverage=0.9, smoothing_sigma=smoothing_sigma, n_dcal=n_dcal, n_classes=n_classes,
    ...
    for calibration_budget in [0.05, 0.1, 0.15, 0.2]:
        ...
        for iter_i in range(n_iterations):
            ...
            eval_mask = ~cal_mask
            start_time = time.time()
            threshold = vanilla_bin_cp.calibrate_from_scores(smooth_scores[cal_mask], smooth_prediction.y_true[cal_mask])
            pred_set = vanilla_bin_cp.predict_from_scores(smooth_scores[eval_mask][:500])
            end_time = time.time()
            pred_set = vanilla_bin_cp.predict_from_scores(smooth_scores[eval_mask])

            empirical_coverage = vanilla_bin_cp.internal_cp.coverage(pred_set, y_true_mask[eval_mask])
            average_set_size = pred_set.sum(dim=1).float().mean().item()

            vanilla_bin_results.append({
                "method": "vanilla-bin", 
                "iteration": iter_i,
                "coverage_guarantee": coverage_guarantee,
                "r": r,
    ...
    vanilla_bin_results = pd.DataFrame(vanilla_bin_results)
    vanilla_bin_results.to_csv(f"{result_folder}/vanilla-bin_results-time-tr_samples{trial_samples}-NOERROR.csv", index=False)
    ...
    cas_results.to_csv(f"{result_folder}/cas_results-time-tr_samples{trial_samples}-NOERROR.csv", index=False)
    bin_results.to_csv(f"{result_folder}/bin_results-time-tr_samples{trial_samples}-NOERROR.csv", index=False)
